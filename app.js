...
// v42 visual-only fallback: traffic-light status classes. Does not move or remove content.
(function () {
  function colorStatuses() {
    document.querySelectorAll(".row-status").forEach((el) => {
      const txt = (el.textContent || "").trim().toLowerCase();
      el.classList.toggle("status-done", txt.includes("registrert") || txt.includes("faktisk inn"));
  ...
  document.addEventListener("DOMContentLoaded", colorStatuses);
  new MutationObserver(colorStatuses).observe(document.documentElement, {
    childList: true,
    subtree: true,
    characterData: true
  });
